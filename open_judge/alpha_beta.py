...
def value(node, alpha, beta):
    ...
    if node.rule == 'max':
        ...
        return minValue(node, float('-inf'), float('inf'))
    ## End your code
...
def maxValue(node, alpha, beta):
    ...
    for successor in successors:
        if successor.rule == 'min':
            v = max(v, minValue(successor, alpha, beta))
        elif successor.rule == 'max':
            v = max(v, maxValue(successor, alpha, beta))
        if v >= beta:
            return v
        alpha = max(alpha, v)
    return v
...
def minValue(node, alpha, beta):
    ...
    for successor in successors:
        if successor.rule == 'min':
            v = min(v, minValue(successor, alpha, beta))
        elif successor.rule == 'max':
