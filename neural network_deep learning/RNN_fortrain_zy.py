...
def transfer_poem(file_name, word_int_map):
    poems = []
    content = ''
    with open(file_name, "r", encoding='utf-8', ) as f:
        for line in f.readlines():
...
def run_training():
    ...
    # poems_vector = transfer_poem('./tangshi.txt', word_to_int)
    ...
    # 构建模型
    end_points = rnn_model(model='lstm', input_data=input_data, output_data=output_targets, vocab_size=len(vocabularies), rnn_size=128, num_layers=2, batch_size=batch_size, learning_rate=0.01)
    ...
    with tf.Session() as sess:
        ...
        #saver.save(sess, './model/finetune/poem_generator_ft')
        saver.save(sess, './poem_generator_1')
...
def gen_poem(begin_word):
    ...
    # poems_vector = transfer_poem('./tangshi.txt', word_int_map)

    input_data = tf.placeholder(tf.int32, [batch_size, None])
    
    end_points = rnn_model(model='lstm', input_data=input_data, output_data=None, vocab_size=len(vocabularies), rnn_size=128, num_layers=2, batch_size=batch_size, learning_rate=0.01)
    # 如果指定开始的字
    if begin_word:
        word = begin_word
    else:
    ...
    with tf.Session() as sess:
        sess.run(init_op)
        saver.restore(sess, './poem_generator_1')# 恢复之前训练好的模型 
        #saver.restore(sess, './model/finetune/poem_generator_ft')
        poem = ''
...
def generate_batch(batch_size, poems_vec, word_to_int):
    ...
    x_batches = []
    y_batches = []
    poems_vec_copy = copy.copy(poems_vec)
    shuffle(poems_vec_copy)
    for i in range(n_chunk):
        start_index = i * batch_size
        end_index = start_index + batch_size

        batches = poems_vec_copy[start_index:end_index]
...
def get_batch(batch_size, poems_vec, word_to_int):
    return 0
...
'''
for head in ['日', '红', '山', '夜', '湖', '海', '月']:
    poem2 = gen_poem(head)# 生成诗歌
    print("#" * 25)
