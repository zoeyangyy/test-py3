...
# mat = dataset['samples']
# print(mat)
# https://blog.csdn.net/loseinvain/article/details/78092613
...
weights = {
    'W1': np.random.uniform(-sqrt(6/(n_input+W1_hidden)), sqrt(6/(n_input+W1_hidden)), size=(n_input, W1_hidden)),
    'W2': np.random.uniform(-sqrt(6/(W1_hidden+W2_hidden)), sqrt(6/(W1_hidden+W2_hidden)), size=(W1_hidden, W2_hidden)),
    'output': np.random.uniform(-sqrt(6/(W2_hidden+n_output)), sqrt(6/(W2_hidden+n_output)), size=(W2_hidden, n_output))
}

biases = {
    'B1': np.zeros(W1_hidden),
    'B2': np.zeros(W2_hidden),
...
def cal_gradient(batch, label):
    ...
    sig31 = np.sum((a3-label)*1)/batch_size
